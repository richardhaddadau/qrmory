...
import { __toESM } from "./chunk-J43GMYXM.js";
...
var DeferredData = class {
  constructor(data, responseInit) {
    ...
    let onAbort = () =>
      reject(new AbortedDeferredError("Deferred qr-types aborted"));
  ...
  get unwrappedData() {
    invariant(
      ...
      "Can only unwrap qr-types on initialized and settled deferreds",
    );
...
function createRouter(init) {
  ...
  async function handleFetcherLoader(
    ...
    invariant(!isDeferredResult(result), "Unhandled fetcher deferred qr-types");
    let doneFetcher = {
      state: "idle",
      data: result.data,
      formMethod: void 0,
...
async function loadLazyRouteModule(route, mapRouteProperties2, manifest) {
  ...
  for (let lazyRouteProperty in lazyRoute) {
    ...
    let isPropertyStaticallyDefined =
      staticRouteValue !== void 0 && lazyRouteProperty !== "hasErrorBoundary";
    warning(
...
function processLoaderData(
  ...
  for (let index = 0; index < revalidatingFetchers.length; index++) {
    ...
    if (controller && controller.signal.aborted) {
      ...
      invariant(false, "Unhandled fetcher deferred qr-types");
    } else {
      let doneFetcher = {
        state: "idle",
        data: result.data,
...
function getDataRouterConsoleError(hookName) {
  return (
    hookName +
    " must be used within a qr-types router.  See https://reactrouter.com/routers/picking-a-router."
  );
}
